      ...
      <table class="min-w-full divide-y divide-default">
        <thead>
          <tr class="text-left text-sm text-muted">
            ...
            <th class="px-4 py-3 font-medium">Cloro</th>
            <th class="px-4 py-3 font-medium">Conductividad</th>
            <th class="px-4 py-3 font-medium">PH</th>
            <th class="px-4 py-3 font-medium">Temperatura</th>
            <th class="px-4 py-3 font-medium">Turbiedad</th>
            <th class="px-4 py-3 font-medium text-right">Acciones</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-default">
          <tr v-if="showInitialLoading">
            <td colspan="9" class="px-4 py-8 text-center text-muted">Cargando reportes...</td>
          </tr>
          <tr v-else-if="!reportes.length">
            <td colspan="9" class="px-4 py-8 text-center text-muted">Todavia no hay reportes registrados.</td>
          </tr>
          <template v-else>
            <tr v-for="reporte in reportes" :key="reporte.id">
              <td class="px-4 py-3">{{ reporte.id }}</td>
              <td class="px-4 py-3 font-mono">{{ periodoLabel(reporte) }}</td>
              <td class="px-4 py-3">{{ centroLabel(reporte) }}</td>
              <td class="px-4 py-3">{{ reporte.cloro }}</td>
              <td class="px-4 py-3">{{ reporte.conductividad }}</td>
              <td class="px-4 py-3">{{ reporte.ph }}</td>
              <td class="px-4 py-3">{{ reporte.temperatura }}</td>
              <td class="px-4 py-3">{{ reporte.turbiedad }}</td>
              <td class="px-4 py-3">
                <div class="flex justify-end gap-2">
                  <UButton icon="i-lucide-pencil" color="neutral" variant="ghost" aria-label="Editar reporte" @click="emit('edit', reporte)" />
                  <UButton icon="i-lucide-trash-2" color="error" variant="ghost" aria-label="Eliminar reporte" @click="emit('delete', reporte)" />
                </div>
              </td>
            </tr>
          </template>
